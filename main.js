...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ==========================================================================
     TAB SWITCHING (ENQUIRE NOW VS CHECK REPLIES)
     ========================================================================== */
  const tabBtnEnquire = document.getElementById('tab-btn-enquire');
  const tabBtnReplies = document.getElementById('tab-btn-replies');
  const checkRepliesPanel = document.getElementById('check-replies-panel');
  ...
  if (tabBtnEnquire && tabBtnReplies) {
    tabBtnEnquire.addEventListener('click', () => {
      tabBtnEnquire.classList.add('active');
      tabBtnReplies.classList.remove('active');
      checkRepliesPanel.classList.remove('active');
    ...
    tabBtnReplies.addEventListener('click', () => {
      tabBtnReplies.classList.add('active');
      tabBtnEnquire.classList.remove('active');
      checkRepliesPanel.classList.add('active');
      ...
      const lastPhone = localStorage.getItem('sscc_last_phone');
      if (lastPhone && lookupPhoneInput) {
        lookupPhoneInput.value = lastPhone;
        performLookup(lastPhone);
      }
    });
  }

  /* ==========================================================================
     ENQUIRY REPLIES LOOKUP & STUDENT-SIDE CHAT LOGIC
     ========================================================================== */
  if (lookupSearchBtn && lookupPhoneInput) {
  ...
  function renderLookupResults(list) {
    ...
    if (list.length === 0) {
      lookupResultsContainer.innerHTML = `
        <div class="search-empty-state">
          ...
          <p>We couldn't find any inquiries associated with this phone number. Make sure the number matches exactly.</p>
        </div>
      `;
      if (typeof lucide !== 'undefined') lucide.createIcons();
      return;
    ...
    lookupResultsContainer.innerHTML = sorted.map(enq => {
      const replies = enq.replies || [];
      ...
      if (enq.status === 'replied') {
        badgeClass = 'replied';
        badgeText = 'Replied';
      } else if (enq.status === 'read') {
        badgeClass = 'read';
        badgeText = 'Read';
      }

      // Format Date
      const d = new Date(enq.timestamp);
      const formattedDate = d.toLocaleDateString('en-IN', { day: '2-digit', month: 'short' }) + ' ' + 
                            d.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' });

      // Thread messages
      const bubblesHtml = replies.map(rep => {
        const isAdmin = rep.sender !== 'user';
        return `
          <div class="chat-bubble ${isAdmin ? 'admin' : 'student'}">
            <div class="chat-bubble-meta">
              ${isAdmin ? '<i data-lucide="shield"></i> Admin' : 'You'} &middot; ${formatTime(rep.ts)}
            </div>
            <div class="chat-bubble-text">${escHtml(rep.text)}</div>
          </div>
        `;
      }).join('');

      return `
        <div class="user-enq-card" id="user-card-${enq.id}">
          <div class="user-enq-hdr" onclick="toggleUserEnqBody('${enq.id}')">
            <div class="user-enq-info">
              <h4>${escHtml(enq.course || 'General Enquiry')}</h4>
              <p>${formattedDate}</p>
            </div>
            <span class="enq-badge ${badgeClass}">${badgeText}</span>
          </div>
          <div class="user-enq-body" id="user-body-${enq.id}">
            <div class="original-msg-label">Your Query</div>
            <div class="original-msg-content ${enq.message ? '' : 'empty'}">
              ${enq.message ? escHtml(enq.message) : 'No message text provided.'}
            </div>
            
            <div class="chat-bubbles" id="user-bubbles-${enq.id}">
              ${hasReplies ? bubblesHtml : `
                <div style="text-align:center; padding:1rem; color:var(--text-muted); font-size:0.8rem; font-style:italic;">
                  Waiting for response from our advisors. We will contact you soon.
                </div>
              `}
            </div>

            <!-- Two-way Reply Form -->
            <div class="user-reply-compose">
              <textarea id="user-reply-input-${enq.id}" placeholder="Type a message to the advisor..." rows="2"></textarea>
              <button type="button" class="send-user-reply-btn" onclick="sendUserReply('${enq.id}')">
  ...
  // Toggle card body open/close
  window.toggleUserEnqBody = function(id) {
